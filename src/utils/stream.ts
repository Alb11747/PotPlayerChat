import { stripSymbols } from './strings'
...
/**
 * Finds a stream from a history of streams by its title.
 * @param title The title of the stream to find.
 * @param streamHistory A list of streams with their URLs and titles.
 * @returns The stream object if found, otherwise null.
 */
export function findStreamByTitle(
  title: string,
  streamHistory: ({ url: string; title: string } | null)[]
): { url: string; title: string } | null {
  return (
    streamHistory.find(
      (stream) => stream?.title && stripSymbols(stream.title) === stripSymbols(title)
    ) || null
  )
}
